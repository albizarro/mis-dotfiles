...
ADDR_FILE="/tmp/hypr_dropdown_terminal_addr"
...
spawn_terminal() {
  ...
  # Get existing window addresses
  local windows_before=$(hyprctl clients -j | jq -r '.[].address' | sort)
  ...
  for i in {1..20}; do
    sleep 0.1
    local windows_after=$(hyprctl clients -j | jq -r '.[].address' | sort)

    # Check for new unique address
    new_addr=$(comm -13 <(echo "$windows_before") <(echo "$windows_after") | head -1)

    if [ -n "$new_addr" ]; then
      debug_echo "Window detected in $((i * 100))ms"
      break
    fi
  done

  if [ -n "$new_addr" ] && [ "$new_addr" != "null" ]; then
    # Force floating and correct properties immediately in case initial rules failed
    hyprctl dispatch setfloating "address:$new_addr"
    hyprctl dispatch resizewindowpixel "exact $width $height,address:$new_addr"
    hyprctl dispatch movetoworkspacesilent "special:scratchpad,address:$new_addr"

    # Store the address and monitor name
    echo "$new_addr $monitor_name" >"$ADDR_FILE"
    debug_echo "Terminal created with address: $new_addr"

    # Small delay to ensure Hyprland processes the state changes
    sleep 0.1
